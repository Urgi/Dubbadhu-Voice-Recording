...
export default function AdminAudioReviewScreen({ navigation }: Props) {
  ...
  const autoStateRef = useRef<{ itemId: string | null; waitingSlowEnd: boolean; waitingFastEnd: boolean }>({
    itemId: null,
    waitingSlowEnd: false,
    waitingFastEnd: false,
  })
  ...
  useEffect(() => {
    if (!current) return
    // Auto-review flow: play slow first, then fast.
    ...
    void (async () => {
      ...
      if (current.fast_audio_url) {
        ...
        await playUrl(current.fast_audio_url, `${current.id}-fast`)
      }
    })()
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.finalActions}>
        ...
        <Pressable style={[styles.approveBtn, saving && styles.disabled]} onPress={() => void approveAndNext()} disabled={saving}>
